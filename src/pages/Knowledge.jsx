...
import { Search, Play, Book, Download, Filter, Clock, Star, Zap, GraduationCap, Code, Briefcase } from 'lucide-react';
        ...
        <div className="sticky top-20 z-30 bg-gray-50/95 dark:bg-ucak-dark/95 backdrop-blur-md py-4 mb-8 -mx-6 px-6 border-b border-gray-200 dark:border-gray-800">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="bg-white dark:bg-ucak-dark-card p-1 rounded-xl border border-gray-200 dark:border-gray-700 flex shadow-sm">
              <button onClick={() => setViewMode('courses')} className={`px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${viewMode === 'courses' ? 'bg-ucak-blue text-white shadow-md' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                <Play size={16} /> Cours
              </button>
              <button onClick={() => setViewMode('documents')} className={`px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${viewMode === 'documents' ? 'bg-ucak-blue text-white shadow-md' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5'}`}>
                <Book size={16} /> Docs
              </button>
            </div>
            ...
            <div className="flex items-center gap-4 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide">
              <div className="flex gap-2">
                {filieres.map((f) => (
                  <button key={f.name} onClick={() => setActiveFiliere(f.name)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border transition-all ${activeFiliere === f.name ? 'bg-ucak-green text-white border-ucak-green' : 'bg-white dark:bg-ucak-dark-card text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-ucak-green'}`}>
              ...
              <div className="flex gap-2">
                {levels.map((lvl) => (
                  <button key={lvl} onClick={() => setActiveLevel(lvl)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeLevel === lvl ? 'text-ucak-blue dark:text-ucak-gold bg-ucak-blue/10 dark:bg-ucak-gold/10 ring-1 ring-ucak-blue/20 dark:ring-ucak-gold/20' : 'text-gray-500 hover:text-ucak-blue dark:hover:text-white'}`}>
                    {lvl}
                  </button>
                ))}
              </div>
                  ...
                  <div className="bg-white dark:bg-ucak-dark-card rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 border border-gray-100 dark:border-gray-800 h-full flex flex-col">
                    <div className="relative h-44 overflow-hidden">
                      ...
                      <div className="absolute top-2 left-2 flex gap-1">
                        <span className="bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-md border border-white/10">{course.level}</span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      ...
                      <div className="mt-auto">
                        ...
                        <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                          <motion.div initial={{ width: 0 }} animate={{ width: `${course.progress}%` }} className="h-full bg-gradient-to-r from-ucak-green to-ucak-blue"></motion.div>
                        </div>
                      </div>
                    </div>
                  </div>
