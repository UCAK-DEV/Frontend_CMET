...
import { 
  Briefcase, Trophy, Users, Zap, Clock, Calendar, 
  ArrowUpRight, BookOpen, MoreHorizontal, Bell 
} from 'lucide-react';
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';

// Widget Rapide (Petit Carré)
...
export default function Dashboard() {
  ...
  const notifications = [
    { id: 1, text: "Note disponible : Algorithmique", time: "2 min" },
    { id: 2, text: "Nouveau stage : Orange Sénégal", time: "1h" },
  ];
    ...
    <div className="min-h-screen pt-28 pb-10 bg-gray-50/50 dark:bg-ucak-dark px-6">
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
           <div>
              <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Espace Étudiant</p>
           ...
           <div className="flex gap-3">
              <button className="p-3 bg-white dark:bg-white/5 rounded-full border border-gray-100 dark:border-white/10 text-gray-500 hover:text-ucak-blue relative">
                 <Bell size={20} />
                 <span className="absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-ucak-dark"></span>
              </button>
              <div className="px-5 py-3 bg-white dark:bg-white/5 rounded-full border border-gray-100 dark:border-white/10 text-xs font-black uppercase tracking-widest text-gray-500 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
           ...
           {/* 1. CARTE ÉTUDIANT (Prend 1 colonne, 2 rangées sur Desktop) */}
           <div className="md:col-span-1 md:row-span-2">
              <StudentCard />
           </div>

           {/* 2. STATS RAPIDES (XP & RANG) */}
           <div className="md:col-span-2 bg-ucak-blue text-white p-8 rounded-[2.5rem] relative overflow-hidden shadow-2xl flex flex-col justify-between min-h-[200px]">
              ...
              <div className="relative z-10 flex justify-between items-start">
                 <div>
                    <h3 className="text-lg font-black opacity-80 mb-1">Niveau d'Expérience</h3>
                    <p className="text-4xl font-black tracking-tighter">{user?.xp_points || 0} XP</p>
                 </div>
                 <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md">
                    <Trophy size={24} className="text-ucak-gold" />
                 </div>
              </div>
              ...
              <div className="relative z-10 mt-6">
                 <div className="flex justify-between text-xs font-bold uppercase tracking-widest opacity-60 mb-2">
                    ...
                    <span>Niveau Suivant: 2000 XP</span>
                 </div>
                 <div className="h-2 bg-black/20 rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: 0 }} 
                      animate={{ width: `${Math.min(((user?.xp_points || 0) / 2000) * 100, 100)}%` }} 
           ...
           {/* 3. PROCHAIN COURS (Timeline Widget) */}
           <div className="bg-white dark:bg-ucak-dark-card p-6 rounded-[2.5rem] border border-gray-100 dark:border-white/5 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-4 text-ucak-gold text-xs font-black uppercase tracking-widest">
              ...
              <div className="mt-auto">
                 <span className="inline-block px-3 py-1 bg-green-100 dark:bg-green-900/20 text-green-600 text-[10px] font-black uppercase rounded-lg">
                    {nextClass.status}
                 </span>
              </div>
           </div>

           {/* 4. ACTIONS RAPIDES (Grille de boutons) */}
           ...
           <div className="md:col-span-1 bg-white dark:bg-ucak-dark-card p-6 rounded-[2.5rem] border border-gray-100 dark:border-white/5">
              <div className="flex justify-between items-center mb-6">
                 <h4 className="font-black text-sm uppercase tracking-widest">Activité</h4>
                 <MoreHorizontal size={16} className="text-gray-400" />
              </div>
              <div className="space-y-4">
                 {notifications.map(notif => (
                   <div key={notif.id} className="flex gap-3 items-start">
                      <div className="w-2 h-2 mt-1.5 rounded-full bg-ucak-blue shrink-0"></div>
                      <div>
                         <p className="text-xs font-bold text-gray-700 dark:text-gray-300 leading-tight">{notif.text}</p>
