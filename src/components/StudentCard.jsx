    ...
    <div className="w-full relative overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-[#0f141e] to-[#1a2333] text-white shadow-2xl border border-gray-700/50 p-6 flex flex-col justify-between h-full min-h-[240px] group transition-transform hover:scale-[1.01]">
      ...
      <div className="relative z-10 flex items-center gap-5 my-5 pl-6">
        <div className="relative group-hover:scale-105 transition-transform duration-500">
           ...
           <div className="absolute -bottom-1 -right-1 bg-ucak-green border-2 border-[#0f141e] rounded-full p-0.5 z-20"><CheckCircle2 size={10} className="text-white" /></div>
        </div>
        <div className="flex-1 min-w-0">
           <h3 className="text-lg font-black uppercase tracking-tight truncate text-white leading-tight">{studentName}</h3>
           <div className="flex items-center gap-2 text-ucak-gold/90 mt-1.5">
              <GraduationCap size={14} />
              <p className="text-[10px] font-bold uppercase tracking-widest truncate">{studentFiliere}</p>
           </div>
           <p className="text-[9px] text-gray-500 font-mono mt-1">VALID: <span className="text-gray-300">{currentYear}</span></p>
        </div>
      </div>
      ...
      <div className="relative z-10 flex justify-between items-end border-t border-white/5 pt-4 pl-6">
         <div>
            <div className="flex items-center gap-1.5 mb-1 opacity-50"><Lock size={8} /><p className="text-[8px] uppercase font-black tracking-widest">ID Crypté</p></div>
            <p className="text-xs font-mono text-white tracking-[0.15em] tabular-nums">{studentMatricule}</p>
         </div>
         <div className="bg-white p-1 rounded-lg shadow-[0_0_15px_rgba(255,255,255,0.1)] relative overflow-hidden">
            ...
            <img src={qrCodeUrl} alt="QR Sécurisé" className="w-12 h-12 object-contain mix-blend-multiply" />
         </div>
      </div>
    </div>
  );
